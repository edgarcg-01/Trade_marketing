...
/**
 * Web Push del Portal B2B (Fase 3, lado cliente).
 *
 *  - `enable()`: pide permiso (gesto del usuario), se suscribe con la clave VAPID
 *    del backend y registra la suscripción en `POST /push/subscribe`.
 *  - `initClicks()`: al tocar una notificación, navega a su `data.url`.
 *
...
export class PushService {
  ...
  readonly supported =
    this.swPush.isEnabled && typeof Notification !== 'undefined';
  ...
  initClicks(): void {
    ...
    this.swPush.notificationClicks.subscribe(({ notification }) => {
      const url = (notification?.data as { url?: string })?.url || '/portal/home';
      this.router.navigateByUrl(url);
      this.telemetry.track('push_click', { url });
    });
  }
  ...
  async enable(): Promise<boolean> {
    ...
    try {
      ...
      if (!enabled || !publicKey) {
        this.telemetry.track('push_unavailable');
        return false;
      }
      ...
      this.telemetry.track('push_enabled');
      return true;
    } catch (err) {
      ...
      this.telemetry.track('push_enable_failed', { reason: (err as Error)?.name });
      return false;
    }
  }
}
