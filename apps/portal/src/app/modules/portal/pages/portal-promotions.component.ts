...
@Component({
  ...
  styles: [
    `
      ...
      .pp-bento-main-cta i { font-size: var(--fs-xs); transition: transform 200ms var(--ease-standard); }
      ...
      .pp-bento-side-cta i { color: var(--brand-700); font-size: var(--fs-xs); }
...
export class PortalPromotionsComponent implements OnInit {
    ...
    forkJoin({
      ...
      .subscribe({
        next: ({ promos, customer, warehouses, priceLists }) => {
          ...
          // E4 — pedimos SOLO los precios de los productos que las promos
          // referencian, no las ~7k filas del price-list completo.
          const neededIds = this.collectPromoProductIds(promos);
          this.api.pricesByIds(pl.id, neededIds, this.warehouseId || undefined).subscribe({
            next: (rows) => {
  ...
  /**
   * Junta los product_ids referenciados por las promos para pedir solo esos
   * precios (E4). Mismo mapeo por tipo que `enrichPromo` — mantener en sync.
   */
  private collectPromoProductIds(promos: PromotionRow[]): string[] {
    const ids: string[] = [];
    for (const p of promos) {
      ...
      switch (p.promotion_type) {
        ...
        case 'volume_discount':
          if (rules.product_id) ids.push(rules.product_id);
          break;
        case 'bundle_fixed_price':
          if (Array.isArray(rules.items)) {
            for (const it of rules.items) if (it.product_id) ids.push(it.product_id);
          }
          break;
        case 'cross_sell_discount':
          if (rules.trigger_product_id) ids.push(rules.trigger_product_id);
          if (rules.target_product_id) ids.push(rules.target_product_id);
          break;
        default:
          break;
      }
