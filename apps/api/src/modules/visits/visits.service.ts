...
export class VisitsService {
  ...
  async checkIn(userId: string, username: string, storeId: string, lat: number, lng: number) {
    const store = await this.knex('stores').where({ id: storeId, activo: true }).first();
    if (!store) throw new NotFoundException('Punto de Venta no existente en base de datos');
  ...
  async checkOut(visitId: string, userId: string) {
    const visit = await this.knex('visits').where({ id: visitId, user_id: userId }).first();
    if (!visit) throw new NotFoundException('Visita no pertenece a este ejecutivo');
    if (visit.status === 'completed') throw new BadRequestException('El Checkout ya había sido disparado');

    // Compilar todas las evaluaciones guardadas en las Exhibitions hijas
    const exhibitions = await this.knex('exhibitions').where({ visit_id: visitId });
    ...
    const finalScore = exhibitions.length > 0 ? (totalScoreSum / exhibitions.length) : 0; // Promedio de perfección de toda la tienda 0-100%
  ...
  async findOne(id: string) {
    ...
    for (const ex of visit.exhibitions) {
      ex.photos = await this.knex('exhibition_photos').where({ exhibition_id: ex.id });
    }
    return visit;
  }
}
