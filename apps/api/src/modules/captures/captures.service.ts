...
export class CapturesService {
  ...
  async create(createCaptureDto: CreateCaptureDto, userId: string, username: string, zona: string) {
    ...
    const [capture] = await this.knex('captures')
      ...
      .returning(['id', 'folio', 'user_id', 'zona_captura', 'fecha_captura', 'kpis_data']);

    return capture;
  }

  async findAll(zona?: string, ejecutivo?: string, fecha_inicio?: string, fecha_fin?: string) {
