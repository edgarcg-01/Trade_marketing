...
export class ReportsComponent implements OnInit {
  ...
  // ── Agrupación por día con semáforo ────────────────────────────
  ...
  exportBuiltPdf() {
    ...
    doc.setFontSize(10);
    doc.text(
      `Período: ${this.filtersState.rangeLabel()} | ${new Date().toLocaleString()}`,
      14,
      y,
    );
    y += 8;
    ...
    if (this.pdfSections.find((s) => s.id === 'metrics')?.checked) {
      doc.setFontSize(12);
      doc.text('KPIs de ejecución', 14, y + 4);
      y += 8;
      autoTable(doc, {
        startY: y,
    ...
    if (this.pdfSections.find((s) => s.id === 'furniture')?.checked) {
      doc.setFontSize(12);
      doc.text('Cumplimiento por mobiliario', 14, y);
      y += 4;
      autoTable(doc, {
        startY: y,
        head: [['Activo', 'Meta', 'Estado']],
        body: this.metasConfig
          .furniture()
          .map((f) => [
            f.label,
            f.target,
    ...
    if (this.pdfSections.find((s) => s.id === 'table')?.checked) {
      doc.addPage();
      doc.setFontSize(12);
    ...
    if (
      ...
      doc.addPage();
      doc.setFontSize(12);
      doc.text('Ranking por vendedor', 14, 14);
      autoTable(doc, {
        startY: 20,
