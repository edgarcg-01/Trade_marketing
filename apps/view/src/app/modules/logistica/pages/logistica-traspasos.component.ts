  ...
  template: `
    <div class="surf-page in">
      ...
      <div class="tr-note">
        ...
        Estas cifras se excluyen a propósito de los reportes de venta (Sell-Out, Ventas por ruta, Command Center). Aquí se muestran por separado para control logístico.
      </div>
      ...
      @if (report(); as r) {
        @if (r.rows.length) {
          <div class="tr-kpis">
            <div class="tr-kpi"><span class="tr-kpi-l">Valor total</span><span class="tr-kpi-v">{{ r.totals.value | currency:'MXN':'symbol-narrow':'1.0-0' }}</span></div>
            <div class="tr-kpi"><span class="tr-kpi-l">Documentos</span><span class="tr-kpi-v">{{ r.totals.docs | number }}</span></div>
            <div class="tr-kpi"><span class="tr-kpi-l">Unidades</span><span class="tr-kpi-v">{{ r.totals.units | number:'1.0-0' }}</span></div>
            <div class="tr-kpi"><span class="tr-kpi-l">Tipos</span><span class="tr-kpi-v">{{ r.by_kind.length }}</span></div>
          </div>
          ...
          @if (r.by_kind.length) {
            <div class="tr-kinds">
              @for (k of r.by_kind; track k.kind) {
                <div class="tr-kind">
                  <span class="tr-kind-l">{{ k.kind_label }}</span>
                  <span class="tr-kind-v">{{ k.value | currency:'MXN':'symbol-narrow':'1.0-0' }}</span>
                  <span class="tr-kind-s">{{ k.share_pct | number:'1.0-1' }}%</span>
                </div>
              }
            </div>
          }
          ...
          <div class="so-actions-bar">
            <span class="text-xs text-content-muted">{{ r.rows.length }} filas · año {{ r.year }}</span>
          ...
          <div class="card-premium card-flat tr-table-card">
            <div class="tr-wrap">
              <table class="tr-table">
                ...
                <tbody>
                  @for (row of r.rows; track row.warehouse_code + row.kind) {
                    <tr>
                ...
                <tfoot>
                  <tr class="tot">
                    ...
                    @for (m of r.months; track m) {
                      <td class="n">{{ r.monthly_totals[m]?.value ? (r.monthly_totals[m].value | currency:'MXN':'symbol-narrow':'1.0-0') : '·' }}</td>
                    }
                    <td class="n b">{{ r.totals.value | currency:'MXN':'symbol-narrow':'1.0-0' }}</td>
                    <td class="n">100%</td>
                    <td class="n">{{ r.totals.docs | number }}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
