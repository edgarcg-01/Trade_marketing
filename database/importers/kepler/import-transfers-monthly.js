...
/**
 * T.2 — Feed de MOVIMIENTOS que NO son venta (traspasos / consolidación interna)
 * por sucursal × tipo × mes → Railway (analytics.transfers_monthly). Base del
 * apartado /logistica/traspasos. Mantiene estos docs FUERA de venta.
 *
 * Tipos (kind) y su código de documento Kepler (kdm1.c2/c3/c4):
 *   consolidacion    = U/D/6   (serie UD06, CONTADO, sin destino) — causaba el ×2 en venta
 *   recepcion        = U/A/50  ("Recepción Traspaso Suc")
 ...
 *
 * Lee los 6 servidores Kepler de sucursal EN VIVO (192.168.x). Como purgan
 * historia, el upsert es ACUMULATIVO con GREATEST (nunca baja un mes ya
 * capturado). Aditivo. NO usa mart.ventas ni toca ventas.
 *
 *   DST_URL=postgresql://…railway node database/importers/kepler/import-transfers-monthly.js          # dry-run
 *   DST_URL=…                     node database/importers/kepler/import-transfers-monthly.js --apply   # commit
 *   ... [--year 2026]
 */
...
// Clasificación de doc-type → kind. Cualquier doc que NO sea venta (c4=10).
const KIND = `CASE
...
(async () => {
  ...
  try {
    ...
    await db.query(`CREATE TABLE IF NOT EXISTS analytics.transfers_monthly (
      ...
      warehouse_id uuid NOT NULL, kind text NOT NULL, month date NOT NULL,
    ...
    await db.query(`CREATE UNIQUE INDEX IF NOT EXISTS uq_transfers_monthly ON analytics.transfers_monthly (tenant_id, warehouse_id, kind, month)`);
    ...
    const all = []; // [warehouse_id, kind, month, units, value, docs]
      ...
      try {
        ...
        const { rows } = await src.query(
          `SELECT ${KIND} AS kind,
                  ...
                  count(DISTINCT h.c6) AS docs,
                  sum(coalesce(d.c9,0))::numeric  AS units,
                  sum(coalesce(d.c13,0))::numeric AS value
             FROM md.kdm2 d
        ...
        for (const r of rows) {
          if (!r.kind) continue;
          all.push([wid, r.kind, r.mes, Number(r.units) || 0, Number(r.value) || 0, Number(r.docs) || 0]);
        }
        const val = rows.reduce((a, r) => a + (Number(r.value) || 0), 0);
        console.log(`  ✅ ${b.db} (${b.code}): ${rows.length} filas · $${Math.round(val).toLocaleString()} (${Date.now() - t0}ms)`);
      } catch (e) {
    ...
    for (let i = 0; i < all.length; i += BATCH) {
      ...
      const vals = chunk.map((_, ri) => { const b = ri * 7; return `($${b+1},$${b+2},$${b+3},$${b+4},$${b+5},$${b+6},$${b+7})`; });
      const params = []; chunk.forEach((row) => params.push(M, row[0], row[1], row[2], row[3], row[4], row[5]));
      const res = await db.query(
        `INSERT INTO analytics.transfers_monthly
           (tenant_id, warehouse_id, kind, month, units, value, docs)
         VALUES ${vals.join(',')}
         ON CONFLICT (tenant_id, warehouse_id, kind, month) DO UPDATE SET
