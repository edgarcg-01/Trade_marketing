...
| **Maat** | **AI de Finanzas (conocimiento + chat + patrones)** | **🔨 EN CURSO — MAAT.0 ✅ 2026-07-06 (ADR-028 aceptado). NO fine-tuning: conocimiento curado + chat tool-use (patrón Thot Chat, cero números del LLM) + motor determinista ~14 detectores (riesgo/error_captura/oportunidad) + aprendizaje Horus-L (L0 memorias → L1 baselines → L2 auto-supresión; colector primero). MAAT.0 = `libs/finance` (boundaries `scope:finance`) + schema `finance.*` 7 tablas RLS forzado (mig 20260706190000/191000, local Batch 139) + perms `FINANCE_AI_CHAT`/`FINANCE_FINDINGS_GESTIONAR` (backfill 15 roles, heredan FINANCE_EXPENSES_VER) + seed 27 entries conocimiento (script `seed-maat-knowledge.js`) + endpoints `/finance/maat/knowledge*`. RLS smoke 0/27/0. **Pendiente: migración+seed a prod + re-login.** Sigue MAAT.1 (balanza fam 1-9 + cadena `expense_doc_chain` vía lineage kdm1 c39 — absorbe GX.4.3b). Plan en [`FASE_MAAT`](docs/IMPLEMENTACION/FASES/FASE_MAAT_FINANZAS_AI.md).** |
