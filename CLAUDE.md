...
| **Maat** | **AI de Finanzas (conocimiento + chat + patrones)** | **🔨 EN CURSO — MAAT.0 + MAAT.3 ✅ 2026-07-06 (ADR-028 aceptado). NO fine-tuning: conocimiento curado + chat tool-use (cero números del LLM) + motor determinista + aprendizaje Horus-L. MAAT.0 = `libs/finance` + schema `finance.*` 7 tablas RLS (Batch 139 local) + perms FINANCE_AI_CHAT/FINANCE_FINDINGS_GESTIONAR (backfill 15 roles) + seed 27 entries conocimiento. MAAT.3 (adelantado) = chat "Pregúntale a Maat": 7 tools deterministas (egresos/serie/proveedor/documento/hallazgos/conocimiento/guardar) + loop tool-use (Haiku, think=Sonnet, deep, vision) + system prompt con las 27 entries en vivo + audit `chat_sessions/messages` + 👍/👎 (colector L2). UI `/finanzas/maat` = réplica del diseño /thot-chat (composer reusado con voz/imagen). Smoke E2E 17/17. **Pendiente prod: migraciones+seed newdb + `ANTHROPIC_API_KEY` en Railway + re-login.** Sigue MAAT.1 (balanza fam 1-9 + cadena kdm1 c39 → tools maat_balanza/maat_pnl/maat_cadena) y MAAT.2 (motor 14 detectores + bandeja). Plan en [`FASE_MAAT`](docs/IMPLEMENTACION/FASES/FASE_MAAT_FINANZAS_AI.md).** |
