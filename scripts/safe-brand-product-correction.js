/**
 * Script SEGURO para corregir productos con marcas incorrectas
 * 
 ...
 * 
 * Uso:
 * - Modo diagnóstico: node scripts/safe-brand-product-correction.js --dry-run
 * - Modo corrección: node scripts/safe-brand-product-correction.js --execute
 */
...
const logFile = path.join(logDir, `brand-correction-${new Date().toISOString().replace(/[:.]/g, '-')}.log`);
...
async function createBackup() {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(backupDir, `brands-backup-${timestamp}.json`),
      JSON.stringify(brandsBackup, null, 2)
    );
    fs.writeFileSync(
      path.join(backupDir, `products-backup-${timestamp}.json`),
      JSON.stringify(productsBackup, null, 2)
    );
    
    log(`✅ Backup creado: ${brands.length} marcas, ${products.length} productos`);
...
async function validateReferentialIntegrity() {
  ...
  try {
    ...
    log('✅ Integridad referencial válida');
    return true;
...
async function detectInconsistencies() {
  log('🔍 Detectando inconsistencias entre productos y marcas...');
...
async function executeCorrections(inconsistencies, productsWithoutBrand) {
  if (DRY_RUN) {
    log('🔍 MODO DRY-RUN: Solo mostrando correcciones propuestas:');
  } else {
    log('⚠️  MODO EJECUCIÓN: Aplicando correcciones...');
  }
  ...
  try {
    ...
    if (DRY_RUN) {
      log(`\n📊 Total de correcciones propuestas: ${inconsistencies.length + productsWithoutBrand.length}`);
    } else {
      log(`\n✅ Total de correcciones aplicadas: ${correctionsCount}`);
    }
...
async function validateAfterCorrections() {
  ...
  log('🔍 Validando estado post-corrección...');
  ...
  try {
    ...
    if (integrityValid) {
      log('✅ Todas las validaciones post-corrección pasaron correctamente');
    } else {
      log('❌ Se detectaron problemas post-corrección');
    }
...
async function main() {
  log('='.repeat(60));
  log('INICIO - Script Seguro de Corrección de Marcas');
  log(`Modo: ${DRY_RUN ? 'DRY-RUN (solo diagnóstico)' : 'EJECUCIÓN'}`);
  log(`Entorno: ${env}`);
  log('='.repeat(60));
  ...
  try {
    ...
    // 2. Validar integridad referencial inicial
    const initialIntegrity = await validateReferentialIntegrity();
    if (!initialIntegrity) {
      log('❌ No se puede continuar: problemas de integridad detectados');
      process.exit(1);
    }
    ...
    // 5. Ejecutar correcciones si hay problemas
    if (diagnosis.inconsistencies.length > 0 || diagnosis.productsWithoutBrand.length > 0) {
      ...
      log('✅ No se encontraron problemas que requieran corrección');
    }
