...
> **Estado: 🔨 PA.0 + PA.1 (a+b) + PA.3 + PA.4 (a+b) EN CÓDIGO — 2026-06-19. Fase PA = núcleo COMPLETO.** Layout de pasillos (schema + backend + editor 2D) + **tablero de asignación por folio** (parejo) + **conteo particionado por pasillo** (items.aisle_id al abrir + avance por pasillo) + **scoping del contador** (cada contador solo cuenta SKUs de su pasillo; sobrante atribuido a su pasillo). Único diferido: PA.4c (lock de pasillo terminado, requiere columna de estado). **Reparto = PAREJO** (el proporcional de PA.2/ADR-024 fue eliminado). Extensión de la Fase I (conteo físico) y de la Fase ABC (conteo cíclico). Organiza el conteo por **pasillos** (zonas 2D del almacén), con **1 supervisor por pasillo** y un **equipo de contadores** por cada uno. Decisión en [ADR-024](../02_DECISIONES_ARQUITECTURA.md).
...
| **PA.4b** ✅ código | Scoping del contador | ✅ 2026-06-19: `submitCount` enforce por pasillo. Un contador es *aisle-scoped* si **todas** sus filas en `inventory_count_assignments` tienen `aisle_id` (una fila NULL = contador folio-wide, modo I.4, sin restricción). Scoped → solo cuenta SKUs cuyo `item.aisle_id` ∈ sus pasillos (si no → 403); SKU no listado (sobrante) se atribuye a su pasillo si cubre **uno** (si cubre varios → 403, que lo mapee el supervisor). Backwards-compat total (folios sin tablero por pasillo = igual que antes). Build api verde + smoke §6 (cuenta A ok / B 403 / sobrante→A). |
| **PA.4c** ⬜ diferido | Lock de pasillo terminado | El supervisor marca su pasillo como "contado/cerrado" y `submitCount` rechaza más conteos de ese pasillo (freeze-por-pasillo). Requiere estado de lock por pasillo (columna/tabla nueva). El scoping de PA.4b ya impide la contaminación entre pasillos (el riesgo real de integridad); esto es un control operativo del supervisor, separable. |
