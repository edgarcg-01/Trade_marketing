...
> **Estado: 🔨 PA.0 + PA.1 (a+b) + PA.2 EN CÓDIGO — 2026-06-19.** Layout de pasillos (schema + backend + editor 2D) + **generador de equipos proporcional**. Siguiente: PA.3 (tablero de asignación 2D por folio — persiste el plan). Extensión de la Fase I (conteo físico) y de la Fase ABC (conteo cíclico). Organiza el conteo por **pasillos** (zonas 2D del almacén), con **1 supervisor por pasillo** y un **equipo de contadores proporcional** a cada uno. Decisión en [ADR-024](../02_DECISIONES_ARQUITECTURA.md).
