...
| **MAAT.1 — Data completa** | Importer `analytics.ledger_monthly` (familias 1-9, 24 meses, 6 sucursales) · importer `analytics.expense_doc_chain` (lineage c39, absorbe GX.4.3b) · cron re-run diario · backend `expenseDocument` devuelve `chain` (UI del timeline **ya está escrita**, hoy dormida) | balanza cuadra vs Kepler en 2 meses spot-check; ≥90% facturas XA2001 con cadena `exact` |
...
| **MAAT.3 — Chat** | `POST /finance/maat/chat` SSE + 9 tools + system prompt + audit `chat_sessions/messages` · UI `/finanzas/maat` + botón contextual en egresos · 👍/👎 por respuesta | 15 golden-questions contestadas con números exactos (verificados vs SQL directo); 0 números inventados |
