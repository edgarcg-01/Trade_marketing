...
### ACT.5 — Balanceo de carga entre rutas/personas ✅ EN CÓDIGO (builds verdes, smoke 10/10)
- **Modelo**: `daily_assignments` es 1 ruta/persona/día → persona = su ruta. Nivelar el tiempo de cada persona = **mover clientes** de la ruta más cargada a una vecina más liviana (redimensionar), manteniendo el # de rutas ("separadas igual"). Decisión Edgar: **ambos** (reasignar + mover fronteras, subsumido en el move de clientes) · **tiempo híbrido** · **co-piloto** (aprueba→escribe, reversible).
- `RouteBalanceService` (`route-balance.service.ts`): tiempo por ruta = **Σ minutos-visita por cliente (observados de `commercial.vendor_visits`, fallback estimado) + traslado NN-Haversine / 18 km/h**. Greedy: mueve el cliente-frontera (más cerca del centroide de la ruta liviana, guard `dist_L ≤ dist_H×1.25`, anti-oscilación) hasta desbalance ≤15 min o tope de movimientos.
- Endpoints: `GET /supervisor-ai/route-balance?day_of_week=` (simula read-only), `POST /route-balance/apply` (SUPERVISOR_AI_APROBAR — recomputa server-side, escribe `commercial.customers.sales_route`+`visit_sequence=null`, guarda `previous_state` en `commercial.route_rebalance_log`), `POST /route-balance/undo` (restaura desde el log). Mig `20260721140000` crea el log (RLS forzado).
- Frontend `route-balance.component.ts` (`/dashboard/supervisor-ai/route-balance`): selector de día, KPIs (ruta más larga hoy vs nivelada, mejora%, σ antes→después, movimientos), tabla tiempo-por-persona con barras antes→después, lista de movimientos (cliente: RUTA A→RUTA B), botones **Aplicar** (confirm) + **Revertir**. Botón "Balanceo de carga" en el header de Horus.
- **Pendiente: verificación en vivo (endpoints route-balance) + validación visual.** Diferido: involucrar vendedores sin asignación del día + reasignar rutas enteras entre personas (hoy subsumido por el move de clientes).
