...
> **Estado:** 🧪 HIQ.0–HIQ.6 EN CÓDIGO 2026-07-15 (builds verdes). HIQ.0/1/2/3a/4 verificados en runtime; HIQ.5/3b/6 con builds + validación puntual (pendiente restart API para el resto). **Fase Horus-IQ COMPLETA (alcance del plan).**
>
> **HIQ.5 ✅ en código** — nudge en vivo al aprobar: `EventsService.emitFieldNudge` emite `horus:nudge` a la room propia del colaborador; `external_delivery` ahora es veraz (`ws`/`deferred`). El pull (`/field/my-*`) sigue siendo la vía durable (vendor offline-first). Listener WS del vendor diferido (bajo valor sin conexión frecuente).
> **HIQ.3b ✅ en código** — cross-foto: pHash dHash-64 con sharp (best-effort, mig `20260715160000`) → regla `vision_recycled_phash` (near-dup Hamming≤8, lo que la URL-exacta no veía) + `shelf_declining` (calidad del anaquel de la tienda cae ≥0.15 vs 2 semanas atrás). Ambas source=vision, detectan/informan sin acusar. dHash validado con sharp contra imagen real.
> **HIQ.6 ✅ en código** — visibilidad de errores: `guard()` registra la sección que falla en `failedSections` + banner "no se pudo cargar X · Reintentar". Mata el "sección caída = sin datos" del audit. Atomización del componente en N hijos diferida (cosmética, el dashboard funciona).
>
> **Migraciones prod pendientes**: `20260715150000` (capture_vision_products/HV.1) + `20260715160000` (phash/HIQ.3b). Las de HIQ.0–2 ya están en Railway. **Restart API pendiente** para: poblar `products_seen`/`phash` (scans nuevos), disparar `over_declaration`/`vision_recycled_phash`/`shelf_declining`, y activar el nudge WS.
>
