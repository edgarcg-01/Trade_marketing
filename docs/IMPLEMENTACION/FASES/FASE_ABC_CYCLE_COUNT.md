...
> **Estado: 🟢 FASE COMPLETA (en código) — 2026-06-19.** ABC.0 clasificación + ABC.1 cycle-due/agenda + ABC.2 folio cíclico acotado + ABC.3a scheduler/cron + **ABC.3b UI** (`/comercial/inventory/abc`). Control de inventario continuo end-to-end: clasificar → ver qué toca → contar solo eso → automatizarlo, con su UI Operations. **Pendiente: 1 reinicio** (verde live de smokes I.6 + I.7) **+ QA visual** de la página. Item estratégico de inventario tras caducidad/FEFO (ver [FASE_I_INVENTARIO.md](FASE_I_INVENTARIO.md) §Roadmap P2). Diferido: ABC.4 (cadencia por tenant, zona, umbral $).
...
| **ABC.3b** ✅ código | UI | ✅ 2026-06-19: página `/comercial/inventory/abc` (tab "Cíclico", gate SUPERVISAR). Superficie **Operations** (DESIGN.md): page-head Hanken bold, **KPI strip** (Por contar / Valor clasificado / barra de Distribución ABC) + **tabla densa** con 2 vistas vía `p-selectButton` — **Agenda** (cycle-due, accionable) y **Clasificación** (ABC por valor). Acciones: **Recalcular ABC** (ghost) + **Generar folios** (sunset, gate por almacén + confirm dialog). `p-tag [severity]` A=success/B=warn/C=secondary, due=danger/warn/secondary; `tabular-nums`; empty states con CTA; in-page hairline sin sombra. Nuevo `GET /commercial/inventory/abc/summary` (agregado para KPIs). Build view+api verde. ⏳ QA visual + reinicio. |
| **ABC.4** ⬜ defer | Refinamientos | policy de cadencia por tenant, asignación por zona, aprobación por umbral $, 2º conteo aleatorio. |

**Orden de valor:** ABC.0 ✅ → ABC.2 ✅ (el corazón) → ABC.1 ✅ → ABC.3a ✅ → ABC.3b ✅ (UI). **Fase completa en código** — solo resta verificación (reinicio + QA visual). Único diferido: ABC.4 (refinamientos).
