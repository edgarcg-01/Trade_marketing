...
| **T.4.0** | **Medición atribuida (cierra la observación del loop)** | `FeedbackService.conversionByReason(days, attribution_days)`: conversión **atribuida a nivel producto** (producto ofrecido → comprado en ventana, vía `order_lines`, NO "cualquier pedido") y **desglosada por `reason`** (whitespace/recompra/afinidad/estrategia/...). Las ofertas se loguean con fidelidad perfecta server-side: `GET thot/suggest/:id?log=<canal>` escribe `commerce_signals.context.items=[{p,r}]`. Endpoint `GET signals/conversion-by-reason`. Take-order (apps/view) loguea 1 vez por apertura. **Es el prerrequisito de T.4**: sin medir qué señal convierte, no se pueden ajustar pesos. Build verde; SQL verificado; métrica forward-looking (se puebla con actividad real). | 🧪 código listo · pend. reinicio API + log en apps/vendor+portal |
| T.4 | Feedback weights | `signal_weights` (tabla, aún NO existe) + recalibración desde T.4.0 + frequency capping. Hoy los pesos son constantes hardcoded → **el motor NO aprende todavía**, solo se actualiza con datos y ahora se mide. | ⬜ |
