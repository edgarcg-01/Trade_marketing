...
> **Estado: 🔨 P2.0 + P2.1a + P2.1b + P2.2a + P2.2b EN CÓDIGO — 2026-06-18.** `commercial.stock_lots` + **trigger del invariante** con FEFO-decrement (verificado vs order flow J.6.1 19/0) + **captura lote/caducidad** en recepción + endpoints `GET .../lots` y `GET .../expiring` + **cron de alerta de lotes por vencer**. **P2.1b + P2.2a ✅ LIVE-verificados (smoke I.5 26/26).** Decisión en [ADR-022](../02_DECISIONES_ARQUITECTURA.md). **P2.2b (alerta) ⏳ pendiente de probar live (reinicio).** Siguiente: **P2.2c** dashboard "Por vencer" + **P2.2d** gate de venta de vencidos (diseño primero).
...
| **P2.2d** ⬜ | Gate de venta de vencidos (**diseño primero**) | Impedir que una venta consuma lotes ya vencidos. **Necesita diseño** (candidato a addendum de ADR-022): el invariante obliga al trigger a decrementar *algo*; hay que (a) excluir lo vencido del "disponible" para vender y/o (b) distinguir venta (FEFO entre no-vencidos) de merma write-off (que sí toca el vencido). No es un edit trivial del trigger. |
...
**Orden de valor real:** P2.0 ✅ → P2.1a ✅ (trigger; ya da el FEFO-decrement) → P2.1b ✅ captura → P2.2a ✅ endpoint `/expiring` → P2.2b ✅ código alerta "por vencer" → **P2.2d gate de vencidos** (correctness; diseño primero) → P2.2c dashboard (visibilidad) → P2.3 (registrar el lote consumido en el ledger) → P2.4 conteo por lote → P2.5 vendedor/portal.
