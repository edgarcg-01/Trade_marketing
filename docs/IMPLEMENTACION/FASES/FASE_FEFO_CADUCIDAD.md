...
> **Estado: 🔨 P2.0 + P2.1a + P2.1b EN CÓDIGO — 2026-06-18.** `commercial.stock_lots` (mig `20260618200000`) + **trigger del invariante** con decremento FEFO (mig `20260618210000`, verificado vs order flow J.6.1 19/0) + **captura lote/caducidad en recepción** (`recordMovement('in')` + endpoint `GET .../lots`, build verde, ⏳ requiere reinicio para probar live). Decisión en [ADR-022](../02_DECISIONES_ARQUITECTURA.md). Siguiente: **P2.2 alertas "por vencer"** (ya hay lotes con caducidad capturables).
