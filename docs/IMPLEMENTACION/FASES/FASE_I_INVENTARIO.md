...
- **Reason-codes estructurados ✅ 2026-06-18** (merma/caducado/dañado/robo/error_conteo/error_sistema/devolución/transferencia/encontrado/otro): `reason_code` en `inventory_count_items` + propagado al ledger (ambas tablas de movimientos), taxonomía validada en servicio (`VARIANCE_REASONS`, mig `20260618180000`), endpoint `variance-reasons` + dropdown `p-select` en el dialog de resolver.
- **KPI de IRA ✅ 2026-06-18** — endpoint `GET /commercial/inventory/counts/ira` sobre folios reconciliados: IRA por piezas (dentro de tolerancia/total) + exactitud por valor + variación neta $ + **shrinkage por causa** (desglose por `reason_code`) + IRA por folio; tolerancia configurable (default 0 = exacto; meta industria >97%). Página `/comercial/inventory/ira` (nav "Exactitud (IRA)", gate SUPERVISAR). **Pendiente:** test con folio reconciliado real (el smoke I.5 valida el shape; reconciliar en single-user choca con la segregación).
- **Tolerancia de varianza + count-back ✅ 2026-06-18** — `recount_threshold_pct` por folio (mig `20260618190000`, default 0 = off). En `computeDiscrepancies`, un item cuyos conteos **coinciden** pero cuya |varianza vs teórico| excede `expected·pct/100` **no se auto-resuelve**: queda en `discrepancy` para forzar recuento/revisión antes de mover el saldo (control estándar: out-of-tolerance ⇒ count-back). Input "Umbral de recuento %" en el dialog de abrir folio. Smoke I.5 cubre el caso fuera-de-tolerancia.

**P1 = 🟢 COMPLETO (beta scope) 2026-06-18** — A3 ledger+costo · reason-codes · KPI de IRA · tolerancia+count-back. Pendientes transversales: reiniciar API para smokes verde, test E2E del modo inventory, y test de IRA con folio reconciliado real (segregación impide reconciliar single-user).
