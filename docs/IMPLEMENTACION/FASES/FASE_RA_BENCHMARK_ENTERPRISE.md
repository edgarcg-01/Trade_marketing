...
**RA-PRO.1 — Safety stock por nivel de servicio.** Reemplazar el "objetivo" estático por `ROP = demanda_diaria × LT + Z × σ_demanda × √LT`. Tenemos ventas diarias (σ calculable) y LT default configurable por proveedor/categoría. Nivel de servicio configurable por clase ABC (A=98%, B=95%, C=90%). *Es el salto individual más grande hacia "profesional" y no depende de nada externo.*

**RA-PRO.2 — Matriz ABC-XYZ.** Sumar el eje **XYZ** (coeficiente de variación de la demanda) sobre el ABC existente. Cada celda define: nivel de servicio, método de pronóstico y cadencia de revisión. Barato, alto impacto, habilita todo lo demás.

**RA-PRO.3 — Lead time real por proveedor.** Ya probamos lectura viva de Kepler (OC `X-A-35` → orden de entrada `X-A-40`). Donde exista gap real de fechas, medimos LT por proveedor y su **variabilidad**. Caveat honesto: como capturan gran parte de la cadena el mismo día, para muchos SKU el LT deberá **capturarse manual** (como el MOQ) hasta tener señal limpia.
