...
- **⚠️ SIN líneas de producto — VEREDICTO FORENSE (Fase B, 2026-07-17):** el desglose por SKU del telemarketing **NO EXISTE en la DB operativa de Kepler**. Barrido exhaustivo de md_00 (CEDIS):
  - **`kdm2`** (líneas, 193k): **cero** filas `U-A-5`. Los únicos géneros de venta con líneas son `U-D-40` (pedidos `PD-`), `U-D-13` (traspasos/salidas) y `U-D-41`; el resto son compras `X-A-*`. Confirmado por `GROUP BY (c2,c3,c4)` all-time y por folio exacto.
  - **`kdue`** (27.8k): es el **estado de cuenta CxC** — una fila por factura keyed por cuenta-cliente (`c2='10-70'` etc.), total en `c11=c17` (= `c16` del header). Sin SKU.
  - **`kdpord`** (4.8k): líneas de pedido con SKU (`c3`) + cantidad (`c9`/`c13`), pero **solo enlazan a `U-D-40`** (`c20='U',c21='D',c22=40`, los 4857). No hay pedidos ligados a `U-A-5`.
  - **`kdm5`** = pagos/formas de pago; **`kdm6`** = distribución contable (cuenta+importe, sin producto).
  - **`kdfe33*` / `kdfe*`** (tablas CFDI 4.0): **TODAS vacías (0 filas)** en CEDIS → el timbrado y sus *conceptos* no se retienen en esta DB.
  - **`kdij`** (kardex por SKU): CEDIS tiene ventas reales con SKU solo en género `U-D` (órdenes `U-D-40/41`, SKUs reales 94094…); el telemarketing (`U-A`) = **1 sola fila** con SKU `'00001'`.
  - **CAUSA RAÍZ:** el telemarketing se contabiliza como **monto global contra pseudo-SKUs de bucket fiscal** — `00001`=`VENTAS AL 0%`, `00002`=`VENTAS AL 16%`, `00003`=`DEVOLUCIONES` (en `kdii`, no productos). Tanto la factura `U-A-5` como la salida `U-D-13` (≈1:1 en pesos por escritorio, verificado jun-2026) y el movimiento de kardex usan estos genéricos. **Kepler NUNCA captura la mezcla de producto del telemarketing** — la info no existe desde el punto de venta.
  - `c45`(kdm1)=`c22`(kdue)=`'6721'` NO es puntero de surtido: es folio reusado entre tipos de doc.
  - **Conclusión:** la factura `U-A-5` es un documento financiero puro (total en `c16`). **Único camino viable para el nivel-SKU = los CFDI *emitidos* (conceptos del XML) vía Descarga Masiva SAT** (pipeline `@nodecfdi` ya existente — ver memoria `reference_sat_descarga_masiva_nodecfdi`), matcheando por RFC receptor + total + fecha contra los headers `U-A-5`. Alternativa: cambio de proceso para que telemarketing capture líneas como el flujo moderno `PD-`.
...
- **Integración a la plataforma.** El feed `import-sales-fact.js`/`mart.ventas` es line-level (SKU) y telemarketing no da líneas → NO entra al Sell-Out por-producto con el pipeline actual. **Además, meter Kepler-telemarketing DUPLICARÍA** el mayoreo que ya vive en prod vía Wincaja (caja 70 = `mayoreo_credito` → canal `wincaja_credito`, renombrado a etiqueta "Mayoreo" 2026-07-17). Por eso: revenue por-canal ("Mayoreo") YA está; el por-producto queda **bloqueado hasta el camino SAT-CFDI** (Fase B'). Se **revirtió** el intento de `c4=5` en las funciones mart (era inerte).
