/**
 * J12.3 — Solver de ruta heurístico (sin dependencias externas).
 *
 * Resuelve un TSP abierto (open-route): parte del origen (CEDIS), visita todas
 * las paradas, NO regresa al origen. Estrategia: nearest-neighbor para una
 * solución inicial + 2-opt para refinar. Distancia = haversine (km).
 *
 * Para decenas de paradas (caso real de un reparto) es instantáneo y da rutas
 * muy cercanas al óptimo. Si algún día se requiere ventanas horarias / capacidad
 * dura, se cambia por OR-Tools o una API de ruteo (ver FASE_J12, J12.3 opción B).
 */
...
export interface SolveResult {
  /** ids de paradas en orden óptimo de visita */
  order: string[];
  /** distancia total km (origen → ... → última parada) */
  total_km: number;
}
...
/** Largo total de una ruta abierta origen → seq[0] → ... → seq[n-1]. */
...
/** 2-opt: invierte segmentos mientras reduzca la distancia total. */
